...
EspNetworkManager::EspNetworkManager(String chipId){    /*Αρχικοποιση Κλάσης EspNetworkManager */
moduleID=chipId;//Depricated
ssid=("ESP"+chipId).c_str();//Κατασκευη Ονοματος WiFi δικτυου απο τον Σεριακο Κωδικο του ESP8266
}
void EspNetworkManager::begin(){ /*Αρχικοποιση Μεταβλητών AcessPoint και WebServer */

  Serial.println("<<<<<<NETMAN STARTED>>>>>>");  //Προβολη των ήδη αποθηκευμένων μεταβλητών(Για αποσφαλμάτωση)
    ...
    ESP8266WebServer server(80);//Αρχικοποιση WebServer που ακουει στην θύρα 80
    local_IP= IPAddress(192,168,4,22);//IP προσπέλασης WebServer (Αρχική σελίδα HTML)
    gateway=IPAddress (192, 168, 4, 9);//Προεπιλεγμένη πύλη τοπικου δικτύου
    subnet=IPAddress (255, 255, 255, 0);//Διευθυνση υποδικτύου
  	Module_Description = "No Description";//Αρχικοποίηση περιγραφής συσκευής
  	initializeAP();//Εναρξη AcessPoint
  	Serial.println("NETMAN>>AccessPoint Initialization completed");
  	initializeWebServer();
}

boolean EspNetworkManager::runManager(){
  while(!managerFinished){//Αν δεν δωθεί εντολή τέλους μέσω της μεταβλητής manager finished
    server.handleClient();//Εξυπηρέτησε τους πελάτες που συνδέονται στον WebServer
  }
  return true;
}

void EspNetworkManager::initializeAP(){           /*Εναρξη λειτουργίας AcessPoint*/
  //Οι 3 παρακάτω εντολές απαιτούνται για την σωστή ρύθμισης της συσκευής ως AccessPoint
	WiFi.mode(WIFI_AP);
	WiFi.disconnect();
  WiFi.mode(WIFI_AP);
	delay(100);
	WiFi.softAPConfig(local_IP, gateway, subnet);//καταχώρηση παραμέτρων για την δημιουργια AccessPoint
	if (WiFi.softAP(ssid))//Εντολή εναρξης AccessPoint.True=επιτυχής έναρξη. False=εναρξη απέτυχε
	{
    Serial.print("NETMAN>>AccessPoint started at: ");
		Serial.println(WiFi.softAPIP());//Προβολη διευθυσνης πύλης
    Serial.println(ssid);//Προβολη ονόματος δικτύου
	}else
	{
...
void EspNetworkManager::initializeWebServer(){/* Η μέθοδος αναλαμβάνει τα γεγονότα δρομολόγησης του WebServer */
    ...
    //Η μεταβλητή "page" κατασκευάζει την HTML σελίδα που απαιτείται
		...
		server.send(200, "text/html",page );//Αποστολή της σελίδας HTML στον browser του χρήστη
	});
	server.on("/options", [this]() {//Δρομολόγηση στην σελίδα ρυθμίσεων MQTT
		int argnumber = server.args();//Αριθμός των url μεταβλητων που χρησημοποιήθηκαν στην κλήση της σελίδας
		if (argnumber > 0) {//Αν υπάρχουν μεταβλητες τοτε αυτες προήλθαν απο την φόρμα Javascript
      //Αρα πατήθηκε αποθήκευση στην σελίδα
      ...
      for (int i = 0; i < argnumber; i++) {//Εμφάνηση των μεταβλητών
			...
			Module_Description = server.arg(0);//Μεταβλήτη Περιγραφή απο τις μεταβλητές URL
			BrokerAddress = server.arg(1);//Μεταβλήτη Διευθυσνη μεσιτη απο τις μεταβλητές URL
      BrokerUsername = server.arg(2);//Μεταβλήτη ονομα χρηστη απο τις μεταβλητές URL
      BrokerPassword = server.arg(3);//Μεταβλήτη κωδικός απο τις μεταβλητές URL
			server.send(200, "text/html", "Options Saved");
			ClearEeprom(2);//Εκαθάριση των προηγουμενων μεταβλητών απο την ROM
			delay(10);
      ...
      WriteEeprom(5);
      WriteEeprom(6);
		}else{//Δεν υπάρχουν μεταβλητές αρα ο χρήστης επισκεπτετε για πρώτη φορά την σελίδα
			String page = FPSTR(HTML_GENERAL_HEADER_STYLE);
	...
	server.on("/wifi", [this]() { //Δρομολόγηση στην σελίδα επιλογή Wi-Fi
      ...
      Serial.println("NETMAN>>Number of properties: " + String(argnumber));
			...
			WIFI_SSID = server.arg(0);//Μεταβλήτη SSID απο τις μεταβλητές URL
			WIFI_PASS = server.arg(1);//Μεταβλήτη Password απο τις μεταβλητές URL
      ...
      //Αποθήκευση μεταβλητών στην ROM
			...
			WriteEeprom(2);
			delay(10);
			managerFinished= true;                  /*Τερματισμός υπηρεσίας ρυθμίσεων.Εξοδος στο κυρίως πρόγραμμα*/
		}else//Δεν υπάρχουν μεταβλητές αρα ο χρήστης επισκεπτετε για πρώτη φορά την σελίδα
		{
			String page = FPSTR(HTML_GENERAL_HEADER_STYLE);
			page += FPSTR(HTML_SSID_SCRIPT);
			page += FPSTR(HTML_SSID_BODY_1);
      int n = WiFi.scanNetworks();//Αναζήτηση δικτύων κοντα στον esp8266
			if (n == 0){//Αν δεν υπάρχουν δικτυα εμφανησε καταλληλο μυνημα στην HTML σελιδα
				...
				String EncryptType;
				String signal;
				for (int i = 0; i<n; i++){//Για καθε στοιχειο στην λίστα δικτύων
					mssid = WiFi.SSID(i);//καταχώρηση του ονόματος δικτύου
					EncryptType = ((WiFi.encryptionType(i) == ENC_TYPE_NONE) ? " " : "*Secure");//καταχώρηση της μορφής κλειδώματος
					signal = String(WiFi.RSSI(i));//καταχώρηση του σήματος δικτύου
          //Προσθήκη των πληροφόριών σε μια καινούργια γραμμή HTML σαν ενα καινουργιο στοιχείο λίστας
	...
	server.onNotFound( [this]() {//Δρομολόγηση σε αγνωστη σελίδα
		Serial.println("Page not Found");
		server.send(404, "text/plain", "FileNotFound");//Αποστολή κωδικού 404 not found
	});
	server.begin();//Εκκίνηση του Webserver με τα παραπάνω γεγονότα
	Serial.println("NETMAN>>WebServer Started.");
}

//Βοηθήτικές μέθοδοι για την λήψη δεδομένων απο την ROM
...
String EspNetworkManager::ReadEeprom (int c){  /*Διαβασμα περιοχής της EEPROM*/
  int addr_pass = 32; //Διεύθυνση Κωδικου δικτύου = 0x20 (Μεγιστος χώρος=16bytes)
	int addr_desc = 48;// Διευθυνση περιγραφής συσκευής= 0x30 (Μεγιστος χώρος=64btyes)
  int addr_brok = 112;//Διευθυνση διευθυσνης μεσίτη = 0x70 (Μεγιστος χώρος=30byte)
  int addr_bruser=142;//Διευθυνση ονοματος χρήστη μεσίτη = 0x70 (Μεγιστος χώρος=25byte)
  int addr_brpass=167;//Διευθυνση κωδικού μεσίτη = 0x70 (Μεγιστος χώρος=40byte)
	String returnString;
	if (c == 1)//Διευθυνση Ονοματος Δικτύου = 0x00 (Μεγιστος χώρος=32bytes)
	{
		String esid = "";
